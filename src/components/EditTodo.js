...
// verification d'erreur de synthase
/*import { useState } from 'react';
...
export default function EditTodo({ todo, updateTodo }) {
  ...
  function handleKeyDown(e) {
    if (e.key === 'Enter' && value.length) {
     tryUpdateTodo({...todo, content:false,edit:false});
      setValue('');
    }
  }
  ...
  return (
    <div className="d-flex justify-content-center align-items-center mb-10">
      ...
      <button onClick={() => tryUpdateTodo({...todo,edit:false})} className="btn btn-reverse-primary">
        Annuler
      </button>
    </div>
  );
